...
export default function AirlinesAdminPage() {
  ...
  async function handleSave(data: Omit<Airline, "id">, id?: string) {
    try {
      if (id) {
        ...
        setAirlines((prev) => (prev.map((a) => (String(a.id) === id || a.code === id ? updated : a))));
      } else {
        // Create new airline
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      const code = deleting.code || deleting.iataCarrierCode || "";
      if (code) {
        await adminApi.airlines.delete(code);
        setAirlines((prev) => prev.filter((a) => (a.code || a.iataCarrierCode) !== code));
      }
      setDeleting(null);
    } catch (error) {
  ...
  const columns: Column<Airline>[] = [
    {
      ...
      render: (a) => (
        <div className="flex items-center gap-2.5">
          <span
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-aviation-900 text-xs font-bold text-white"
          >
            {a.code}
          </span>
    ...
    },
    {
      ...
      render: (a) => (
        <span className="flex items-center gap-1.5 text-sm text-slate-500">
          <Mail className="h-3.5 w-3.5" /> {a.contactEmail || "N/A"}
        </span>
      ),
    },
    {
      ...
      render: (a) => (
        <span className="flex items-center gap-1.5 text-sm text-slate-500">
          <Phone className="h-3.5 w-3.5" /> {a.contactPhone || "N/A"}
        </span>
      ),
    },
    {
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No airlines match your search."
        />
      )}
      ...
      <ConfirmDeleteModal
        open={!!deleting}
        itemLabel={deleting ? deleting.name || deleting.operatorName || "" : ""}
